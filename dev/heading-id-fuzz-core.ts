...
import { healHeadlessContainersInSlice } from '../src/editor/paste-plugin.js';
...
const enterChain: Command[] = [
  enterAtTagEnd, enterAtZoneStart, enterMidTag, neverThrow(enterInHeading),
  neverThrow(baseKeymap['Enter']!),
];
const backspaceChain: Command[] = [
  crossContainerDeleteSelection, backspaceAtTagStart, backspaceAtFirstBodyStart,
  keepCursorInLeadingBlockOnBlockedMerge, neverThrow(blockBackspaceNodeSelect),
  neverThrow(baseKeymap['Backspace']!),
];
const deleteChain: Command[] = [
  crossContainerDeleteSelection, deleteAtTagEnd, deleteAtContainerEnd,
  keepCursorInLeadingBlockOnBlockedMerge, neverThrow(blockDeleteNodeSelect),
  neverThrow(baseKeymap['Delete']!),
];
...
export function runHeadingIdFuzz(seeds: number, opsPerSeed: number): FuzzResult {
  let findings = 0;
  const detail: string[] = [];
  ...
  let state = EditorState.create({
    doc: genDoc(rng),
    plugins: [history(), typeOverBoundaryPlugin, headingIdGuardPlugin],
  });
  const trace: string[] = [];
  ...
  for (let op = 0; op < opsPerSeed; op++) {
    ...
    try {
      if (kind === 'enter') {
        ...
        if (rng() < 0.4) { if (!setSel(target, target + int(rng, 1, 30))) continue; }
        else setNear(target);
        opLabel = `paste[${a}-${b}]@${state.selection.from}-${state.selection.to}`;
        // The app's handlePaste collapses unfittable selections first,
        // probes slice fit, and falls back to a closed+healed slice.
        if (!prepareSelectionForReplace(view)) continue;
        try {
          dispatch(state.tr.replaceSelection(reids));
        } catch {
          const closed = healHeadlessContainersInSlice(new Slice(reids.content, 0, 0));
          dispatch(state.tr.replaceSelection(closed));
        }
      } else if (kind === 'undo') {
        undo(state, dispatch);
      } else {
      ...
      detail.push(`seed ${seed} op#${op} [${opLabel}] THREW: ${(err as Error).message} | trace: ${trace.slice(-6).join(' → ')}`);
      break;
    }
    trace.push(opLabel);
    ...
    if (problems.length) {
      ...
      detail.push(`seed ${seed} op#${op} [${opLabel}]: ${problems[0]}${problems.length > 1 ? ` (+${problems.length - 1} more)` : ''} | trace: ${trace.slice(-6).join(' → ')}`);
      break; // one finding per seed keeps output readable
    }
  }
}
  return { findings, detail, kinds: findingKinds };
}
