...
export const CATEGORY_TABS: {
  ...
  // Sends taglines to a PolicyDebateFlow flow via outbound network calls
  // from the Electron main-adjacent renderer — desktop-only for now (see
  // flow-send.ts), so hidden on web rather than shown empty.
  { id: 'policyDebateFlow', label: 'PolicyDebateFlow', electronOnly: true },
