 ...
 *
 * Every window's toggle is entirely local: before reloading, the
 * toggling window journals its OWN open doc(s) — and only its own;
 * other windows (of either mode) are untouched — and writes this
 * marker to sessionStorage. After the reload, the startup-recovery
 * flow reads it back and auto-opens EXACTLY the docs listed — and
 * only those. Journals from other sessions (crash leftovers) are
 * left in the store for the recovery sidebar on the next normal
 * launch; sweeping them into the new layout would make stale docs
 * reappear on every toggle.
 *
